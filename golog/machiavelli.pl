...
initially(free(C), true) :- card(C), member(C, [c7D, c8D, c9D, c4D]).
...
proc(control(projection), projection).



proc(projection, [add_to_numpile(c4D, c4C), build_numpile(c7D, c8D, c9D), ?(some(c, free(c)))]).



proc(debug2, add_to_numpile(c4D, c4C)).
